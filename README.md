...
- Поведение по комментариям настраивается через `app/event_settings.json` (см. ниже).
...
------------------------------
- Файл `app/event_settings.json` управляет реакциями на события GitHub и GitLab.
- Ключи для каждого провайдера:
  - `commit_comment` — добавлять ли комментарии при обработке коммитов.
  - `branch_comment` — добавлять ли комментарии при создании ветки.
- Значение `true` включает комментарии, `false` — отключает (смена статусов при этом остаётся активной).
- Значения по умолчанию: GitHub — комментарии для коммитов и веток включены; GitLab — комментарии добавляются только при создании ветки.
- После изменения файла перезапустите сервис, чтобы настройки применились.
